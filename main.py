...
def main():
    ...
    with sentry_sdk.isolated_scope() as isolated_scope:
        ...
        assert sentry_sdk.Scope.get_current_scope().get_merged_scope_data() == {"gtag1": "g1", "gtag2": "g2", "itag2": "i1", "tag2": "1", "tag3": "3"}  # this is what sentry_sdk.capure_event() sends.
        sentry_sdk.capture_event({"name": "event3"})

    scope = sentry_sdk.get_current_scope()
    scope.set_tag("tagx", "x")
    assert scope.get_tags() == {"tagx": "x"}
    assert sentry_sdk.Scope.get_current_scope().get_merged_scope_data() == {"gtag1": "g1", "gtag2": "g2", "tagx": "x"}  # this is what sentry_sdk.capure_event() sends.
