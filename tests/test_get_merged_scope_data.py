...
def test_global_scope_data():
    ...
    with new_scope() as scope:
        ...
        assert scope.get_merged_scope_data() == {
    ...
    with isolated_scope() as isolation_scope:
        ...
        assert isolation_scope.get_merged_scope_data() == {
...
def test_global_scope_data_override_current():
    ...
    with new_scope() as current_scope:
        ...
        assert current_scope.get_merged_scope_data() == {
            "tags": {"tag1": "value_global"}
        }
        ...
        assert current_scope.get_merged_scope_data() == {
            "tags": {"tag1": "value_current_scope"}
        }
        assert isolation_scope.get_merged_scope_data() == {
            "tags": {"tag1": "value_global"}
        }
        assert global_scope.get_merged_scope_data() == {
            "tags": {"tag1": "value_global"}
        }
...
def test_global_scope_data_override_isolation():
    ...
    with new_scope() as current_scope:
        ...
        # Read operations should be done on current scope, because others return unintuive results
        assert current_scope.get_merged_scope_data() == {
            "tags": {"tag1": "value_isolation_scope"}
        }
        assert isolation_scope.get_merged_scope_data() == {
            "tags": {"tag1": "value_isolation_scope"}
        }
...
def test_global_scope_data_override_current_plus_additional(additional_type):
    ...
    with new_scope() as current_scope:
        ...
        assert current_scope.get_merged_scope_data(additional_data=additional_data) == {
            "tags": {"tag1": "value_additional"}
        }
        assert isolation_scope.get_merged_scope_data(
            additional_data=additional_data
        ) == {"tags": {"tag1": "value_additional"}}
        assert global_scope.get_merged_scope_data(additional_data=additional_data) == {
            "tags": {"tag1": "value_additional"}
        }
