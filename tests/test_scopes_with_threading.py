...
class MyThread(threading.Thread):
    def __init__(self, custom_current_scope=None, custom_isolation_scope=None, set_current_scope=None):
    ...
    def _run(self):
        ...
        if self.set_current_scope:
            if self.custom_current_scope is not None and self.custom_isolation_scope is None:
                assert event_payload["tags"] == {"tag2": "mythread_custom_value"}
            elif self.custom_current_scope is not None and self.custom_isolation_scope is not None:
                assert event_payload["tags"] == {"tag1": "main_thread_isolated_value", "tag2": "mythread_custom_value"}
        else:
...
def test_scope_data_in_threads_current(set_current_scope):
    ...
    thread = MyThread(custom_current_scope=current_scope, set_current_scope=set_current_scope)
    thread.start()
    thread.join()
